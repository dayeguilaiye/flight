...
export function LlmApiTesterPage() {
  ...
  const activeTargetLabel = tester.selectedModels.length > 0 ? tester.selectedModels.map((model) => model.name).join(' + ') : tester.guest.modelName ? `${tester.guest.modelName} · 临时配置` : '尚未选择 model'
        ...
        <section className="grid flex-1 gap-6 py-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
          <form className="rounded-2xl border border-ink/15 bg-paper/70 p-6 sm:p-8" onSubmit={submitConfiguration}>
            ...
            {tester.authenticated && tester.providers.length > 0 && <div className="mt-6 space-y-2 rounded-xl bg-white/40 p-3">
              <p className="flex items-center justify-between px-2 text-xs font-semibold uppercase tracking-[0.16em] text-ink/45"><span>Saved models</span><span>{tester.selectedModelIds.length ? `${tester.selectedModelIds.length} selected` : 'Select one or more'}</span></p>
              {tester.providers.flatMap((provider) => provider.models.map((model) => <div className="flex items-center gap-2" key={model.id}><button className={`flex min-w-0 flex-1 items-center justify-between rounded-lg px-3 py-2 text-left text-sm transition ${tester.selectedModelIds.includes(model.id) ? 'bg-ink text-paper' : 'hover:bg-ink/5'}`} onClick={() => { tester.toggleModel(model.id); setProviderName(provider.name); setProviderDescription(provider.description); setBaseUrl(provider.baseUrl); setModelName(model.name); setInterfaceType(model.interfaceType); setMaxConcurrency(model.maxConcurrency?.toString() ?? ''); setToken('') }} type="button"><span className="truncate">{provider.name} / {model.name}</span><span className="ml-3 shrink-0 text-xs opacity-60">{interfaceLabels[model.interfaceType]}</span></button><button aria-label={`删除 ${model.name}`} className="rounded-full px-2 py-1 text-xs text-ink/35 hover:bg-signal/10 hover:text-signal" onClick={() => void tester.deleteOwnerModel(model.id)} type="button">×</button></div>))}
